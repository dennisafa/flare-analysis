...
def computegeorge (flux, time):
    ...
    kernel = kernels.CosineKernel(log_period=np.log(2.6), axes=0) * kernels.ExpSquaredKernel(metric=0.5)
    gp = george.GP(kernel)
...
def remove_flares(flare):
    ...
    while len(fd.flaredetectpeak(flare.flux, std=None)) > 0:# while flares are still being detected, compute its model and subtract flares
